...
export function invoiceSecondsRemaining(bolt11, nowMs = Date.now()) {
  try {
    ...
    // light-bolt11-decoder surfaces expiry either as a top-level field or an
    // `expiry` section; absent => the BOLT11 default of 3600s.
    const decoded = decode(bolt11);
    const rawExpiry = decoded?.expiry ?? sections.find((s) => s.name === "expiry")?.value ?? 3600;
