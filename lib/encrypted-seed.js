...
export async function writeMnemonicBackupFile(mnemonic, { dir } = {}) {
  ...
  // Atomic + exclusive. A partial mnemonic file would be worse than no file —
  // the operator might mistake it for a complete backup — and the random
  // filename plus link(2)-exclusive create make collisions impossible to
  // silently lose.
  await atomicCreateExclusive(path, contents);
  return path;
}
