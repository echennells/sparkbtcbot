...
//   loadMnemonicFromEnv()
//     → wraps loadMnemonic, reads SPARK_PASSPHRASE (or SPARK_PASSPHRASE_FILE) /
//       SPARK_SEED_PATH from env
//   rekeyEncryptedSeed({ passphrase, newPassphrase, path? })
//     → re-encrypts seed.enc (mnemonic + sealed policy) under a new passphrase
//   DEFAULT_SEED_PATH
...
import { validatePolicyObject } from "./policy.js";
...
// The shape ({ dailyBudgetSats, maxPerTxSats, allowedOps, allowedRecipients,
// expiresAt, exec }) and its validation live in lib/policy.js; unknown keys
// throw (misspelled-option doctrine: a typo'd policy key must not silently
// mean "no policy").
...
// Change the passphrase on an existing seed file IN PLACE: decrypt under the
// current passphrase, re-encrypt the same mnemonic (and the same sealed
// policy, untouched) under the new one with a fresh salt and IV, atomic swap.
// Nothing else under ~/.spark is passphrase-bound (the ledger HMAC key and the
// leaf-vault derive from the MNEMONIC), so this is the whole rekey. It does
// NOT protect copies of the old file elsewhere — a backup or synced copy still
// opens with the old passphrase; if the FILE may have been copied, the answer
// is rotate, not rekey. Returns { version, policy } of the rewritten file.
export async function rekeyEncryptedSeed({ path = DEFAULT_SEED_PATH, passphrase, newPassphrase }) {
  if (typeof newPassphrase !== "string" || newPassphrase.length < MIN_PASSPHRASE_CHARS) {
    throw new Error(`new passphrase must be at least ${MIN_PASSPHRASE_CHARS} characters`);
  }
  if (newPassphrase === passphrase) {
    throw new Error("new passphrase is identical to the current one — nothing to do");
  }
  ...
  return { version: payload.policy ? 2 : 1, policy: payload.policy };
}
...
// SPARK_PASSPHRASE_FILE: read the passphrase from a file instead of the
// environment — what systemd's LoadCredential and Docker/Kubernetes secrets
// hand you, and the option that never appears in /proc/[pid]/environ or a
// child's inherited env. Exactly like the env var, it is consulted once per
// process; SPARK_PASSPHRASE (if also set) wins so an explicit override works.
// Trailing newlines are stripped (every secrets tool writes one); nothing else
// is trimmed — a passphrase may legitimately start or end with a space.
function readPassphraseFile(path) {
  let raw;
  try {
    raw = readFileSync(path, "utf8");
  } catch (err) {
    const e = new Error(`SPARK_PASSPHRASE_FILE (${path}) is unreadable: ${err?.code ?? err?.message ?? err}`);
...
export async function loadSeedPayloadFromEnv({ clearEnv = true } = {}) {
  const passphrase = process.env.SPARK_PASSPHRASE
    || (process.env.SPARK_PASSPHRASE_FILE ? readPassphraseFile(process.env.SPARK_PASSPHRASE_FILE) : undefined);
  if (!passphrase) {
