...
let loadedSeedContext = null;
export function getLoadedSeedContext() {
  return loadedSeedContext;
}
...
export async function loadSeedPayloadFromEnv({ clearEnv = true } = {}) {
  ...
  loadedSeedContext = payload.policy
    ? { policy: payload.policy, ledgerHmacKey: deriveLedgerHmacKey(payload.mnemonic) }
    : null;
  return payload;
}
