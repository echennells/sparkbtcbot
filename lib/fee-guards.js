...
// How many sats to DEPOSIT on L1 to end up able to pay a Lightning invoice
// through the on-ramp. This flow crosses TWO fee boundaries and the obvious
// "deposit = invoice + lightning fee" UNDER-funds every time: when the deposit
// is claimed into Spark the SSP takes a CLAIM SPREAD, so the amount CREDITED to
// Spark is `deposit − spread`. The spread is not knowable until the deposit has
// 3 confirmations (the claim quote refuses before then), so there is no exact
// pre-quote. Deposit with a conservative buffer, then pay from whatever
// actually credited; never hand the user a single exact "send N" that omits a
// leg.
//
//   depositSats = invoiceSats + lightningFeeSats + claimSpreadBufferSats + slackSats
//
// Two defaults matter for staying a FLOOR (both fail-open if wrong):
//   - lightningFeeSats defaults to the amount-aware `lightningFeeCap` for the
//     invoice, NOT 0 — omitting the Lightning leg is the exact bug this fixes.
//   - claimSpreadBufferSats defaults PROPORTIONALLY (5% of the target, floored
//     at 500). A flat 500 covers the measured ~2.87% spread only near the size
//     it was measured at (~10k); the SSP fee is percentage-shaped (capped at
//     `maxFeePct`=10%), so a large deposit needs a proportional buffer.
// Returns { depositSats, breakdown }. Still a FLOOR WITH MARGIN, not exact —
// after the claim, verify the real credited amount `quote.creditAmountSats`
// (NOT deposit minus it — that difference is the spread) covers invoice +
// Lightning fee before paying.
export function estimateOnrampDeposit(options = {}) {
  rejectUnknownKeys("estimateOnrampDeposit", options, ["invoiceSats", "lightningFeeSats", "claimSpreadBufferSats", "slackSats"]);
  const { invoiceSats, lightningFeeSats, claimSpreadBufferSats, slackSats } = options;

  if (typeof invoiceSats !== "number" || !Number.isFinite(invoiceSats) || invoiceSats <= 0) {
    throw new Error(`estimateOnrampDeposit: invoiceSats must be a positive number of sats, got ${JSON.stringify(invoiceSats)}`);
  }
  // Optional numeric params: undefined => a safe default; anything PRESENT that
  // isn't a finite non-negative NUMBER throws. Not `Number(v)` — that coerces
  // "", null, true, [n] into small values that would silently UNDER-buffer.
  const num = (name, v, fallback) => {
    if (v === undefined) return fallback;
    if (typeof v !== "number" || !Number.isFinite(v) || v < 0) {
      throw new Error(`estimateOnrampDeposit: ${name} must be a non-negative number, got ${JSON.stringify(v)}`);
    }
    return v;
  };

  const lnFee = num("lightningFeeSats", lightningFeeSats, lightningFeeCap({ amountSats: invoiceSats }));
  const target = invoiceSats + lnFee;
  const spread = num("claimSpreadBufferSats", claimSpreadBufferSats, Math.max(500, Math.ceil(0.05 * target)));
  const slack = num("slackSats", slackSats, 0);

  return {
    depositSats: Math.ceil(invoiceSats + lnFee + spread + slack),
    breakdown: { invoiceSats, lightningFeeSats: lnFee, claimSpreadBufferSats: spread, slackSats: slack },
  };
}
...
// The spend-side twin of estimateOnrampDeposit: the LARGEST gift-card face
// value a balance can actually pay for, once every leg is counted. The
// recurring first-spend mistake is the mirror of the on-ramp one — the wallet
// shows "$100 of sats" and the agent tries to buy a $100 card: the merchant's
// markup and the Spark→Lightning fee push the invoice past the balance and the
// pay fails after the user has already said yes.
//
// Cost of a face value F (in the merchant's fiat) is:
//   invoiceSats = F × (1 + merchantMarkupBps/10000) / pricePerBtc × 1e8
//   total       = invoiceSats + Lightning fee (lightningFeeCap on invoiceSats) + slack
// and the answer is the largest F with total ≤ availableSats — then SNAPPED
// DOWN to what the product actually sells: the largest fixed pack ≤ F when
// `denominations` is given (Amazon.com-style: 5/10/20/50/100/200/500), or F
// floored to `range.step` and clamped to [range.min, range.max] for a custom-
// amount product (Walmart-style). Returns null `face` when nothing fits.
//
// Everything is a FLOOR WITH MARGIN: markup and fee bps default to today's
// observed figures (Bitrefill ~2% on many cards; Spark→LN 0.25% + routing,
// which lightningFeeCap already over-caps to 0.5% with a 25-sat floor), and the
// live merchant quote must STILL pass checkInvoiceAgainstQuote before paying —
// this helper picks the size to ask for; it does not authorize the pay.
export function maxSpendableFace(options = {}) {
  rejectUnknownKeys("maxSpendableFace", options, [
    "availableSats", "pricePerBtc", "merchantMarkupBps", "slackSats", "denominations", "range",
  ]);
  const { availableSats: rawAvailable, pricePerBtc, merchantMarkupBps, slackSats, denominations, range } = options;
  // SparkAgent.getBalance() hands back `sats` as a STRING and the raw SDK as a
  // bigint; both are what a caller will naturally pass here. Coerce those two
  // shapes only — anything else still throws (a NaN would silently size to 0).
  const availableSats =
    typeof rawAvailable === "bigint" ? Number(rawAvailable)
    : typeof rawAvailable === "string" && /^\d+$/.test(rawAvailable.trim()) ? Number(rawAvailable)
    : rawAvailable;
  const pos = (name, v) => {
    if (typeof v !== "number" || !Number.isFinite(v) || v <= 0) {
      throw new Error(`maxSpendableFace: ${name} must be a positive number, got ${JSON.stringify(v)}`);
    }
    return v;
  };
  const nonneg = (name, v, fallback) => {
    if (v === undefined) return fallback;
    if (typeof v !== "number" || !Number.isFinite(v) || v < 0) {
      throw new Error(`maxSpendableFace: ${name} must be a non-negative number, got ${JSON.stringify(v)}`);
    }
    return v;
  };
  const available = pos("availableSats", availableSats);
  const price = pos("pricePerBtc", pricePerBtc);
  const markupBps = nonneg("merchantMarkupBps", merchantMarkupBps, 200);
  const slack = nonneg("slackSats", slackSats, 100);
  if (denominations !== undefined && range !== undefined) {
    throw new Error("maxSpendableFace: pass denominations (fixed packs) OR range (custom amount), not both");
  }

  // Sats the merchant invoice may reach so that invoice + fee + slack fits.
  // lightningFeeCap is monotone in the amount, so walk down from the naive
  // bound until it fits (a handful of iterations; the fee is ≤ 0.5% + floor).
  const totalFor = (invoiceSats) => invoiceSats + lightningFeeCap({ amountSats: invoiceSats }) + slack;
  let invoiceCeiling = Math.floor(available - slack);
  while (invoiceCeiling > 0 && totalFor(invoiceCeiling) > available) invoiceCeiling -= 1;
  if (invoiceCeiling <= 0) return { face: null, reason: "balance does not cover the Lightning fee floor plus slack", breakdown: { availableSats: available, slackSats: slack } };

  // Face value (fiat) whose marked-up invoice is exactly invoiceCeiling sats.
  const rawFace = (invoiceCeiling / 1e8) * price / (1 + markupBps / 10_000);

  let face;
  if (Array.isArray(denominations)) {
    const packs = denominations.filter((d) => typeof d === "number" && Number.isFinite(d) && d > 0).sort((a, b) => a - b);
    face = packs.filter((d) => d <= rawFace).pop() ?? null;
  } else if (range) {
    const min = pos("range.min", range.min);
    const max = pos("range.max", range.max);
    const step = nonneg("range.step", range.step, 0.01) || 0.01;
    const floored = Math.floor(rawFace / step + 1e-9) * step;
    const clamped = Math.min(max, floored);
    face = clamped >= min ? Number(clamped.toFixed(8)) : null;
  } else {
    face = Number(Math.floor(rawFace * 100) / 100);
    if (face <= 0) face = null;
  }

  if (face === null) {
    return { face: null, reason: `largest affordable face is ${rawFace.toFixed(2)}, below the smallest product size`, rawFace, breakdown: { availableSats: available, invoiceCeilingSats: invoiceCeiling, slackSats: slack, merchantMarkupBps: markupBps } };
  }
  const invoiceSats = Math.ceil((face * (1 + markupBps / 10_000)) / price * 1e8);
  const lightningFeeSats = lightningFeeCap({ amountSats: invoiceSats });
  return {
    face,
    rawFace,
    breakdown: {
      ...
      lightningFeeSats,
      slackSats: slack,
      totalSats: invoiceSats + lightningFeeSats + slack,
      leftoverSats: available - invoiceSats - lightningFeeSats,
      merchantMarkupBps: markupBps,
      pricePerBtc: price,
    },
  };
}
