...
// is claimed into Spark the SSP takes a CLAIM SPREAD, so the credited balance
// is `deposit − spread`, not `deposit`. And the spread is not knowable until
// the deposit has 3 confirmations (the claim quote refuses before then) — so
// there is no exact pre-quote to be had. Deposit with a conservative buffer,
// then pay from whatever actually credited; never hand the user a single exact
// "send N" that silently omits the claim leg.
//
//   depositSats = invoiceSats + lightningFeeSats + claimSpreadBufferSats + slackSats
//
// claimSpreadBufferSats defaults to 500 (measured ~297 on a 10,350-sat deposit;
// it is flat-ish, so round up). Returns { depositSats, breakdown }. The result
// is a FLOOR WITH MARGIN, not a precise figure — after the claim, verify the
// real credited balance (deposit − quote.creditAmountSats spread) covers the
// invoice + Lightning fee before paying.
export function estimateOnrampDeposit(options = {}) {
  ...
  const { invoiceSats, lightningFeeSats = 0, claimSpreadBufferSats = 500, slackSats = 0 } = options;
  const inv = Number(invoiceSats);
  if (!Number.isFinite(inv) || inv <= 0) {
    throw new Error(`estimateOnrampDeposit: invoiceSats must be a positive number of sats, got ${JSON.stringify(invoiceSats)}`);
  }
  const parts = { lightningFeeSats, claimSpreadBufferSats, slackSats };
  ...
  const lnFee = Number(lightningFeeSats);
  const spread = Number(claimSpreadBufferSats);
  const slack = Number(slackSats);
  return {
    depositSats: Math.ceil(inv + lnFee + spread + slack),
    breakdown: { invoiceSats: inv, lightningFeeSats: lnFee, claimSpreadBufferSats: spread, slackSats: slack },
  };
}
