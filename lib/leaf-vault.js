...
// Atomic, crash-safe write: temp file → fsync → rename. A half-written vault is
// the only thing between the agent and its recovery data, so never leave a
// partial file; the rename is the atomic commit point. The vault is plain JSON
// (hex strings + metadata), so no custom codec is needed.
