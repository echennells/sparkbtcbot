...
| CLI | `@bitrefill/cli` (≥0.3.0) | Shell-driven checkout. **Guest checkout works unregistered** — `search-products` → `buy-products` → `get-invoice-by-id` need no login or API key (verified live 2026-07). Shell-driven means **you** build the command line from catalog values: pass them as argv / a JSON file, validate ids against `^[A-Za-z0-9._-]+$` first, never interpolate — `merchant-spending.md` §1a |
...
**Guest still means an email.** `buy-products` in `@bitrefill/cli` (0.3.0, verified 2026-09-15) declares `--email` as a **required option**: omit it and the command fails with `error: required option '--email <value>' not specified` before any order exists. So "no account" is true and "no email" is not — every guest order carries an address for the receipt. Three consequences: (1) **ask for the email in the same message as the price preview**, so a "yes" can complete the purchase in one turn instead of stalling at checkout — the first live funnel run paused exactly there; (2) that address is PII going into a merchant database (`merchant-spending.md` §3), so it is the user's to give, not yours to guess — **never invent, reuse from context, or default to an address the user hasn't offered for this purchase**; (3) a user who wants no receipt can give any mailbox they control — say so rather than implying "anonymous" means "no email".
