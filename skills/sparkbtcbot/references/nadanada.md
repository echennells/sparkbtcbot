...
**Re-probed 2026-07-29 — mostly true:** advertised `fixed_*` names now purchase successfully, including regional ones (`fixed_1GB_7D_NORTHAMERICA` / `north-america` → $1.19, the deal that didn't exist two days earlier). The `esim_*_V2` names still work too, so both namespaces are live. **Coverage was still incomplete when checked:** `fixed_1GB_7D_EUROPE` / `europe` returned `bundle_slug_mismatch` (stable across repeat probes ~20s apart) while `fixed_2GB_15D_EUROPE` on the same slug succeeded — the catalog still advertises the failing SKU at $1.19. That may simply be a rollout still in progress rather than a permanent gap, which is the point: **treat any of these observations as a snapshot, not a spec.**
